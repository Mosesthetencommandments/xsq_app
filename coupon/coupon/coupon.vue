<template>
	<view class="all">
		...
		<view class="none" v-if="isShow">
			<img src="../static/images/APP-qie4_0000.png" mode="" />
			<view>暂时还没有优惠券噢！</view>
		</view>
		<!-- 有数据的时候的页面 -->
		<view class="box">
			<view class="coupon-box" v-for="(item,index) in coupons" :key="index">
				<img src="../static/images/APP-qie16_0007_-sb.png" alt="" v-if="item.coupon_status === 1 && ordersQuery.coupon_status !== 3 || item.coupon_status === 2 && ordersQuery.coupon_status !== 3" class="couponImg">
				<img src="../static/images/APP-qie16_0006_-sh.png" alt="" v-if="ordersQuery.coupon_status === 3">
				...
				<view class="rightInfo">
					...
					<view class="rightButton" v-if="item.coupon_status === 1 && ordersQuery.coupon_status !== 3">
						<button class="toUse" @click="gotoUse(index)">
							<text>去使用</text>
						</button>
						<button open-type="share" class="toShare" @click="goShareCoupon(item)">
							<text>分享</text>
						</button>
					</view>
					<view class="haveUse" v-if="item.coupon_status === 2 && ordersQuery.coupon_status !== 3">
						<img src="../static/images/APP-qie16_0005tag.png" alt="">
					</view>
				</view>
		...
		<view class="getCoupon" v-if="fullReduction.length > 0 && ordersQuery.coupon_status === 1">
			<view class="getCouponBox" v-for="(item,index) in fullReduction" :key="index">
				<img src="../static/images/APP-qie16_0008_-sO.png" alt="" >
		...
		<!-- <view class="mask" v-if="isShare" @click.stop="isShare = false">
			<view class="QRCodeImg" @click.stop="isShare = true">
				<img :src="QRcode_Img" alt="">
			</view>
			<view 
				class="getShareWeb"
				@click.stop="getWebUrl"
				>
...
export default {
	...
	onLoad(option) {
		...
		// 	uni.setStorageSync('wx_type', option.wx_type);
		// }
	...
	onShareAppMessage(res){
		...
		return{
			 title: '分享优惠券',
			 path: '/wx_login/wx_login?wx_pid=' + customerInfo.customer_id + '&wx_type=3'
		}
	},
    data() {
        return {
    ...
    methods: {
		async getWebUrl(){
			let [err, res] = await this.$http.post('/api/shop/getShortUrl',{
				long_url:config.webUrl1 + '/#/pages/coupon/coupon'
			});
			if(res.data.code === 200){
				uni.setClipboardData({
				    data: res.data.data.splicing_domain_name,
				    success: function () {
		...
		async getShopCustomerCouponList(){
			this.coupons = []
		...
		goSearch(index){
			for(let i = 0 ; i < this.coupons.length; i++){
				if(index === i){
					...
					uni.navigateTo({
						url:'/index-search/index-search/index-search?ids=' + this.Arr
					})
				}else{
		...
		goRedPacket(){
			uni.navigateTo({
				url:'/red-packet/red-packet/red-packet'
			})
		},
		gotoUse(index){
			for(let i = 0 ; i < this.coupons.length; i++){
