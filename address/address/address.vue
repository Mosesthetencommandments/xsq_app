		...
		<view class="address-box">
			<view class="address-item" v-for="(item,index) in arr" :key="index">
				<view class="img-box" v-if="item.is_default === 1">
					<image src="../static/images/APP-qie4_0006.png" mode=""></image>
				</view>
		...
		methods:{
			goEditAddress(e){
				if(this.notGoPay === true){
					uni.navigateTo({
						url:'/editAddress/editAddress/editAddress?&dontJump=1'
					})
				}else if(this.d_integral=='dxs'){
					uni.navigateTo({
						url:'/editAddress/editAddress/editAddress?d_integral=dxs'
					})
				}else{
					uni.navigateTo({
						url:'/editAddress/editAddress/editAddress'
					})
				}
			},
			goSettle(e){//去商品结算
				...
				if(this.d_integral == 'dxs'){
					uni.redirectTo({
					   url:"/settle-accounts/settle-accounts/Point_mall_settlement?id="+e.id+'&d_integral=dxs'
					});
					...
					uni.redirectTo({
					   url:"/settle-accounts/settle-accounts/settle-accounts?id="+e.id
					});
				}
			...
			goEait(e){//去编辑地址
				if(this.notGoPay === true){
					uni.redirectTo({
					   url:"/editAddress/editAddress/editAddress?modification=modification&id="+e.id+'&dontJump=1'
					});
				}else if(this.d_integral == 'dxs'){
					uni.redirectTo({
					   url:"/editAddress/editAddress/editAddress?modification=modification&id="+e.id + '&d_integral=dxs'
					});
				}else if(this.warehouse === true){
					uni.redirectTo({
					   url:"/editAddress/editAddress/editAddress?modification=modification&id="+e.id+'&warehouse=warehouse'
					});
				}else{
					uni.redirectTo({
					   url:"/editAddress/editAddress/editAddress?modification=modification&id="+e.id
					});
				}
			},
