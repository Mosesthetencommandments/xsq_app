	...
	<view>
		...
		<view v-else>
			...
			<view class="compile">
				...
				<view class="compile-right" @click="removeGods" v-if="shopState === 1">
					<p>编辑</p>
				</view>
						...
						<view class="shopcar">
							...
							<view class="shopcar-center">
								<view class="center-left">
									<img src="../static/images/son_0019_right1.png" alt="" v-if="shopItem.isClick === true" @click="changeClickT(index)">
							...
							<view class="shopcar-bottom">合计：<text>￥ {{Math.round((shopItem.shop_goods.actual_price*shopItem.number) * 100) / 100}}</text></view>
						</view>
					</view>
				</view>
			</view>
			...
			<view class="settlement-box">
				<view class="settlement">
					...
					<view class="settlement-right">
						<view class="js" size="mini" @click="pay" v-if="shopState === 1">结算</view>
	...
	export default {
		...
		onLoad() {
			if (!uni.getStorageSync('token')) {
				this.shopgods = uni.getStorageSync("gods")
			} else {
				this.getUserShopCar()
			}
		},
		methods: {
			...
			remove(i) {
				let gods = uni.getStorageSync('gods');
				gods.splice(i, 1);
				// localStorage.setItem('gods', JSON.stringify("gods"));
				uni.setStorageSync('gods', JSON.stringify("gods"));
			},
						...
						if (uni.getStorageSync('token')) {
							...
							for (const v in add) {
								if (add[v].shop_goods.id === this.shopgods[index].shop_goods.id) {
									...
									uni.setStorageSync('gods', JSON.stringify(add));
									return
								}
							}
						}
