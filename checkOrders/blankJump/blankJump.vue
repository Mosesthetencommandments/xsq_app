		...
		methods:{
			async goCheckOrders(){
				...
				if(res.data.code == 200){
					uni.redirectTo({
						url:'/checkOrders/checkOrders/checkOrders?orders_id='+this.orders_id
					})
				}
			},
			async goCheckOrders1(){
				...
				if(res.data.code == 200){
					uni.redirectTo({
						url:'/checkOrders/checkOrders/checkOrders?orders_id='+this.orders_id + '&shop_goods_id=' + this.shop_goods_id
					})
				}
			}
		}
