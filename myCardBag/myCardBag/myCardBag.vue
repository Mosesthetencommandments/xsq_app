	...
	<view>
		<view class="bag-box">
			...
			<view class="bag-title">
				...
				<view class="img-box" @click="getBack"><img src="../static/images/APP-qie-out.png"></view>
			</view>
			<!-- 卡包 -->
				<!-- 银行卡 -->
			<view class="bag-card">
			...
			<view class="bag-card">
				<view class="card-item" v-for="(item,index) in WalletList1" :key="index">
					<image src="../static/images/APP-qie10_0000icon56x56.png" mode=""></image>
					<view class="card-name">
	...
	export default {
		...
		methods:{
			...
			cardRedact(item){//银行卡编辑
				uni.navigateTo({
				    url: '/addBankCard/addBankCard/addBankCard?shop_customer_wallet_id=' + item.id + '&status=' + item.status
				});
			},
			cardRedact1(item){//支付宝编辑
				uni.navigateTo({
				    url: '/creatAlipay/creatAlipay/creatAlipay?shop_customer_wallet_id=' + item.id + '&status=' + item.status
				});
			},
			goAddBankCard(){
				...
				uni.navigateTo({
				    url: '/addBankCard/addBankCard/addBankCard'
				});
			},
			getBack(){
				uni.navigateBack({
			...
			goCreatAlipay(){
				...
				uni.navigateTo({
					url:'/creatAlipay/creatAlipay/creatAlipay'
				})
			}
		}
	}
