<template>
	<view>
		...
		<view class="cardType-box" v-if="isCardType" @click.stop="isCardType = false">
			<view class="cardType">
				...
				<view class="cardTypeItem" v-for="(item,index) in WalletList" :key="index" @click.stop="selectImage(index)">
					<view class="c-left">
						<image src="../static/images/APP-qie10_0000icon56x56.png" mode="" v-if="item.bank_type_pic === undefined"></image>
						...
						<view class="title-box">
							...
							<view class="note">{{item.bank_type_name ? "24小时内到账" : '(1000 以下提现推荐使用)'}}</view>
						</view>
					</view>
		...
		<view class="bankCard" @click="isCardType = true">
			<view class="card" v-if="kk === 1">
				请选择提现方式
			</view>
			<view class="card" v-else>
				<image src="../static/images/APP-qie10_0000icon56x56.png" mode="" v-if="bank_type_pic === null"></image>
				<image :src="bank_type_pic" mode="" v-else></image>
				{{bank_type_name}}（{{card_number}}）
			</view>
			<view class="arrow-box"><image class="arrow" src="../static/images/APP-qie4_0001.png" mode=""></image></view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			goAccount(){
				uni.navigateTo({
					url:'/myCardBag/myCardBag/myCardBag'
				})
			},
			asds(){
