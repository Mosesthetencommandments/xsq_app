<template>
    <view>
		...
		<view class="none" v-if="isShow">
			<image src="../static/images/APP-qie4_0000.png" mode=""></image>
...
export default {
    ...
    methods: {
		...
		gotologistics(id){
			uni.navigateTo({
				url:"/logistics-details/logistics-details/logistics-details?orders_id=" + id 
			})
		},
		...
		goOrders(items){
			if(items.shop_orders_status === 2){
				uni.navigateTo({
				    url: '/blankJump/blankJump/blankJump?orders_id=' + items.id
				});
			}else if(items.shop_orders_status ===3){
				uni.navigateTo({
				    url: '/checkOrders/checkOrders/checkOrders?orders_id=' + items.id
				});
			}
		}
	}
