	...
	<view>
		<view class="header">
			<view class="header-search-left">
				<input class="search"  v-model="searchInfo.goods_name" :placeholder="Index_goods_name">
...
<script>
	export default {
		...
		onLoad(option) {
			...
			if(option.index_goods_name){
				this.Index_goods_name = option.index_goods_name
				this.CommonList1()
			}
		...
		methods: {
			async CommonList1(){
				let [err,res] = await this.$http.post("/api/shop_goods/getShopGoodsCommonList",{
					goods_name:this.Index_goods_name
				});
