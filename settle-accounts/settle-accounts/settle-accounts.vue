						...
						<view class="renRecipients-content">
							...
							<view class="renRecipients-content-footer">
								...
								<view class="renRecipients-content-footer-remark" v-else @click="openCouponList">
									...
									<view v-else>
										<text>选择优惠</text>
										<text class="coupon">{{coupon_name}}满{{require_total_pay}}可用</text>			
										<image src="../static/images/APP-qie4_0001.png" mode=""></image>
									</view>
		...
		methods:{
			...
			async goLockOrders(){//锁定预处理订单  微信支付
				...
				if(res.data.code == 200 && res.data.data.message === 'ok'){
					...
					uni.redirectTo({
					    url: '/checkOrders/blankJump/blankJump?orders_id='+this.orders_id
					});
					uni.redirectTo({
					    url: '/checkOrders/blankJump/blankJump'
					});
				} else if (res.data.code == 200 && res.data.data.message === 'stock_empty') {
					uni.redirectTo({
					    url: '/sellOut/sellOut/sellOut'
					});
				}else{
					uni.showModal({
					    title: '提示',
