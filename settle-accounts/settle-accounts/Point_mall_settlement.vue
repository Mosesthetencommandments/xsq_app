		...
		methods:{
			...
			async goLockOrders(){//锁定预处理订单  微信支付
				...
				if(res.data.code == 200 && res.data.data.message === 'ok'){
					...
					uni.redirectTo({
					    url: '/checkOrders/blankJump/blankJump?orders_id='+this.orders_id + '&shop_goods_id=' + shop_goods_id
					});
			...
			async yeGoLockOrders(){//锁定预处理订单   余额支付
				...
				} else if (res.data.code == 200 && res.data.data.message === 'stock_empty1') {
					uni.redirectTo({
					    url: '/sellOut/sellOut/sellOut'
					});
				}else{
			...
			goAddress(){//去设置收货地址那
				uni.navigateTo({
					url:"/address/address/address?d_integral=" + this.d_integral
				})
			},
