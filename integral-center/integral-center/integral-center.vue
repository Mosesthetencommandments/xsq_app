<template>
	<view class="integral-center">
			...
			<view class="integral-center-chooseCard-content" >
				<view v-for="(item,index) in choosecard" :key="index" class="integral-center-chooseCard-box">
					<view class="integral-center-chooseCard-item" @tap="changetap(index)" :class="{'active':integralIndex ===index}">
						...
						<!-- <img src="../static/images/APP-qie5_0007_jifen-1.png"> -->
		...
		<!-- 兑换中心 -->
		<view class="integral-goods-box_box" v-if="integralIndex === 0">
		...
		<!-- 已兑换 -->
		<view v-if="integralIndex === 1" class="conversion">
		...
		<!-- 排行榜 -->
		<view v-if="integralIndex === 2" class="leaderboard" @click="getCustomerRankInfo">
		...
		methods: {
			...
			async goPoint_mall_settlement(id){
				...
				if(res.data.code === 200){
					...
					uni.navigateTo({
						url: '/settle-accounts/settle-accounts/Point_mall_settlement?shop_goods_id=' + id + '&d_integral=dxs'
					});
				}
